...
double* single_layer(double *x, int N, double (*W)[R], double b, double *y) {	
	int i,j;
...
int main(int argc, char *argv[]) {
	...
	// create a activation
	double activation[N];
	memcpy(activation, x, N*sizeof(double));
	...
	for(int t=1; t<K; t++) {
		...
		memcpy(activation, y, layer_len * sizeof(double));
	}
